...
def _apply_profile_override() -> None:
    ...
    # 1. Check for explicit -p / --profile flag. Keep historical broad
    # scanning, except for child argv after `hermes mcp add --args`.
    value_flags = {
        "-z", "--oneshot",
        "-m", "--model",
        "--provider",
...
def _select_zai_endpoint(current_base: str) -> str:
    """Present an explicit endpoint picker for Z.AI setup."""
    ...
    print("Select Z.AI / GLM endpoint:")
    choices = [f"{label} ({url})" for label, url in options]
    choices.append("Custom proxy URL")

    selected = _prompt_provider_choice(choices, default=default_idx)
    if selected is None:
        return current_base
...
def _detect_concurrent_hermes_instances(
    ...
    # that spawns python.exe (the interpreter that runs our code), so the
    # launcher must not be reported as a concurrent instance. Unrelated
    # ancestors are intentionally not excluded.
...
""",
    ...
    _setup_parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Provider to configure directly (for example: honcho, mem0, hindsight)",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            try:
                # vacuum() merges FTS5 segments first and returns the number
                # of indexes that accepted the optimize command.
                optimized = db.vacuum()
            except Exception as e:
            ...
            for src in ["cli", "feishu", "api_server", "cron"]:
