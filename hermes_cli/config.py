...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    Running inside a container is not sufficient evidence for the published
    Docker install. Supported Docker images stamp ``.install_method`` as
    ``docker``; unstamped manual/container installs should continue through
    the git/pip detection path.
    """
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        "docker_env": {},
        # Reuse Docker backend containers across Hermes processes by task/profile.
        # Opt in explicitly; the default remains cleanup-on-exit to avoid
        # accumulating long-running sleep-infinity containers on small hosts.
        "docker_persist_across_processes": False,
        # Conservative startup janitor for old exited Hermes-labeled containers.
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    for max_key in ("max_output_tokens", "max_tokens"):
        max_tokens = entry.get(max_key)
        if isinstance(max_tokens, int) and max_tokens > 0:
            normalized[max_key] = max_tokens
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    # Only rewrite the old default value. A user who customized ttl_hours
    # should keep their explicit choice.
    if current_ver < 25:
        config = read_raw_config()
