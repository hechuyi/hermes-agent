...
def _stable_service_working_dir() -> str:
    """Return a service cwd that survives source-checkout relocation."""
    try:
...
def _launchd_domain() -> str:
    """Return the launchd domain that can manage the gateway service."""
...
# Per-platform config for this Feishu-specialized runtime.
_PLATFORMS = [
    {
        "key": "feishu",
        "label": "Feishu / Lark",
...
def _all_platforms() -> list[dict]:
    """Return setup-menu platforms for this Feishu runtime fork."""
    return [dict(p) for p in _PLATFORMS]


def _platform_status(platform: dict) -> str:
    """Return a plain-text status string for the Feishu setup menu."""
    if platform.get("key") != "feishu":
...
def _builtin_setup_fn(key: str):
    """Resolve the interactive setup function for this Feishu runtime fork."""
    return {
...
def _configure_platform(platform: dict) -> None:
    """Run the interactive setup flow for a Feishu platform entry."""
    fn = _builtin_setup_fn(platform["key"])
    if fn is not None:
        fn()
        return

    print_warning(f"Unsupported platform in this Feishu runtime fork: {platform['key']}")
