...
Modular wizard with independently-runnable sections:
  ...
  4. Messaging Platforms — connect Feishu / Lark
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    The wizard no longer prompts for credential rotation, vision backends,
    or TTS providers here. Those paths now use safe defaults and are exposed
    through dedicated commands when users need to tune them.
    """
    ...
    # Persist the refreshed config back through the wizard's save path.
    save_config(config)
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # Gateway cwd defaults to home; sudo stays off. Tune later via
        # `hermes setup terminal` or config.yaml.
        ...
        # Image and resource limits use defaults; tune later via
...
_HIGH_IMPACT_KIND_KEYWORDS = {
    ...
    "feishu": "⚠ Feishu/Lark — this will point Hermes at your OpenClaw Feishu/Lark bot",
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    a messaging platform. Applies sensible defaults for everything else; the
    user can customize later via ``hermes setup <section>``.
    """
    ...
    # underlying login/model save) writes via its own load/save cycle, and the
