...
def parents_blocking_ready(
    ...
    """Return parent tasks that prevent ``task_id`` from entering ready."""
    rows = conn.execute(
...
def set_status_direct(
    ...
    """Set a task status directly while preserving Kanban run invariants.

    This supports operator-driven moves that are not expressed by the
    structured verbs such as complete, block, or archive. Moving a task off
    ``running`` closes the active run as reclaimed so attempt history remains
    coherent.
    """
    with write_txn(conn):
        prev = conn.execute(
