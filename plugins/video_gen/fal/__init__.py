...
def _build_payload(
    ...
    *,
    ...
    if clamped is not None and family.get("durations"):
        ...
        # Some families require a unit suffix ("4s" not "4").
...
def _get_managed_fal_video_client(managed_gateway):
    """Reuse the managed FAL client so its internal httpx.Client is not leaked."""
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    """Submit a FAL video request using direct credentials or the managed gateway."""
    _load_fal_client()
...
def _check_fal_video_available() -> bool:
    """True if the FAL.ai video backend is reachable through direct or managed mode."""
...
class FALVideoGenProvider(VideoGenProvider):
    ...
    def is_available(self) -> bool:
        ...
        except Exception:  # noqa: BLE001 — never break provider discovery
            return False
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        return success_response(
            ...
            duration=(
                int("".join(c for c in payload["duration"] if c.isdigit()) or "0")
