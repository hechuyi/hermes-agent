...
def detect_audio_environment() -> dict:
    ...
    try:
        ...
        try:
            ...
            if not devices:
                if host_audio_forwarding:
                    notices.append("No PortAudio devices detected but host audio forwarding is set -- continuing")
                elif termux_capture:
            ...
            # recording/playback works fine. Don't block if host audio is forwarded.
            if host_audio_forwarding:
                notices.append("Audio device query failed but host audio forwarding is set -- continuing")
            elif termux_capture:
