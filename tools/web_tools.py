...
def _get_backend(capability: str = "search") -> str:
    ...
    # actually grant web-search access. Free-tier backends (searxng /
    # brave-free / keyless parallel / ddgs) trail the keyed ones.
    backend_candidates = (
...
async def web_extract_tool(
    ...
    normalized_urls = []
    for _url in urls:
        _normalized_url = normalize_url_for_request(_url)
        if (
            _PREFIX_RE.search(_url)
            or _PREFIX_RE.search(unquote(_url))
            or _PREFIX_RE.search(_normalized_url)
            or _PREFIX_RE.search(unquote(_normalized_url))
        ):
            return json.dumps({
                "success": False,
        ...
        normalized_urls.append(_normalized_url)
