...
class TestDDGSBackendWiring:
    ...
    def test_auto_detect_prefers_keyless_parallel_over_ddgs(self, monkeypatch):
        ...
        # when the ddgs package is installed. ddgs is search-only, so Parallel
        # is preferred so both search and extract work; ddgs remains reachable
        # via an explicit web.backend=ddgs.
        from tools import web_tools
        monkeypatch.setattr(web_tools, "_load_web_config", lambda: {})
        for key in ("FIRECRAWL_API_KEY", "FIRECRAWL_API_URL", "PARALLEL_API_KEY",
