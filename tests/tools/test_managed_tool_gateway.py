...
def test_is_managed_tool_gateway_ready_skips_refresh_for_expired_cached_token(
    ...
    def _record_refresh(refresh_skew_seconds=120):
        refresh_calls.append(refresh_skew_seconds)
        return "fresh-token"

    monkeypatch.setattr("hermes_cli.auth.resolve_nous_access_token", _record_refresh)

    with (
        patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "nousresearch.com"}, clear=False),
        patch.object(managed_tool_gateway, "managed_nous_tools_enabled", return_value=True),
    ):
