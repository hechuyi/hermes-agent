...
class TestFindAgentBrowser:
    ...
    def test_skips_non_runnable_current_path_candidate(self):
        """A stale PATH hit should fall through to the next runnable candidate."""
        def mock_which(cmd, path=None):
        ...
        def mock_runnable(path):
            return path == "/opt/homebrew/bin/agent-browser"

        with patch("shutil.which", side_effect=mock_which), \
             patch("tools.browser_tool.agent_browser_runnable", side_effect=mock_runnable), \
             patch("os.path.isdir", return_value=True), \
             patch(
                 "tools.browser_tool._discover_homebrew_node_dirs",
                 return_value=[],
