...
def test_docker_config_migrate_backs_up_and_migrates_unversioned_config(tmp_path: Path) -> None:
    ...
    (tmp_path / ".env").write_text("OPENROUTER_API_KEY=test\n", encoding="utf-8")
...
def test_docker_config_migrate_blocks_invalid_yaml_without_rewrite(tmp_path: Path) -> None:
    config_path = tmp_path / "config.yaml"
    ...
    assert "failure_class=config_parse_failed" in proc.stderr
    assert "stage=read_config" in proc.stderr
...
def test_docker_config_migrate_skip_env_leaves_config_unchanged(tmp_path: Path) -> None:
    config_path = tmp_path / "config.yaml"
    original = yaml.safe_dump({"_config_version": 0})
    config_path.write_text(original, encoding="utf-8")
...
def test_docker_config_migrate_blocks_schema_incompatible_config(tmp_path: Path) -> None:
    config_path = tmp_path / "config.yaml"
    ...
    assert proc.returncode != 0
    assert "failure_class=config_schema_incompatible" in proc.stderr
    assert "stage=read_config" in proc.stderr
    assert "action=abort" in proc.stderr
    assert config_path.read_text(encoding="utf-8") == "- not\n- a\n- mapping\n"
    assert not list(tmp_path.glob("*.bak-*"))
