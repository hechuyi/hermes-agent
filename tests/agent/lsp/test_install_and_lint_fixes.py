...
def test_lsp_client_wraps_windows_cmd_and_bat_shims():
    from agent.lsp.client import LSPClient

    assert LSPClient._win_wrap_cmd(["server.cmd", "--stdio"]) == [
        "cmd.exe",
        "/c",
        "server.cmd",
        "--stdio",
    ]
    assert LSPClient._win_wrap_cmd(["server.bat"]) == ["cmd.exe", "/c", "server.bat"]
    assert LSPClient._win_wrap_cmd(["server.exe", "--stdio"]) == [
        "server.exe",
        "--stdio",
    ]
