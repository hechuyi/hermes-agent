...
def _dummy_openai_api_key(suffix: str = "session") -> str:
    return "sk-" + f"{suffix}fixturevalue123456"


def _dummy_anthropic_api_key(suffix: str = "api") -> str:
    return "sk-" + "ant-" + "api-" + f"{suffix}fixturevalue123456"


def _dummy_anthropic_oauth_token(suffix: str = "oauth") -> str:
    return "sk-" + "ant-" + f"{suffix}fixturevalue123456"
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def test_redacts_api_key_in_user_message(self, agent, tmp_path):
        agent._session_json_enabled = True
        agent.logs_dir = tmp_path
        api_key = _dummy_openai_api_key("user")
        messages = [
            {"role": "user", "content": f"My key is {api_key} please use it"},
        ]
        agent._save_session_log(messages)
    ...
    def test_redacts_system_prompt_credentials(self, agent, tmp_path):
        agent._session_json_enabled = True
        agent.logs_dir = tmp_path
        api_key = _dummy_openai_api_key("system")
        agent._cached_system_prompt = f"Use key {api_key} for API calls"
...
class TestBuildApiKwargs:
    ...
    def test_kimi_coding_endpoint_sends_max_tokens_and_default_thinking(self, agent):
        """Kimi endpoint sends max_tokens=32000 and defaults to thinking only."""
        agent.provider = "kimi-coding"
        agent.base_url = "https://api.kimi.com/coding/v1"
        agent._base_url_lower = agent.base_url.lower()
    ...
    def test_moonshot_endpoint_sends_max_tokens_and_default_thinking(self, agent):
    ...
    def test_moonshot_cn_endpoint_sends_max_tokens_and_default_thinking(self, agent):
...
class TestConcurrentToolExecution:
    ...
    def test_sequential_browser_type_callbacks_redact_api_key_even_when_global_redaction_disabled(self, agent, monkeypatch):
    ...
    def test_concurrent_browser_type_callbacks_redact_api_key_even_when_global_redaction_disabled(self, agent, monkeypatch):
        monkeypatch.setattr("agent.redact._REDACT_ENABLED", False)
        secret = "sk-proj-ABCD1234567890EFGH"
        tc = _mock_tool_call(
            name="browser_type",
    ...
    def test_concurrent_blocked_write_skips_checkpoint(self, agent, monkeypatch):
        ...
        tc2 = _mock_tool_call(name="read_file", arguments='{"path":"other.py"}', call_id="c2")
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tc1, tc2])
        messages = []
    ...
    def test_concurrent_blocked_write_does_not_steal_slot_from_allowed_write(
        ...
        """A blocked write must not consume the allowed write's checkpoint slot."""
        tc1 = _mock_tool_call(
            name="write_file",
...
class TestAnthropicCredentialRefresh:
    def test_try_refresh_anthropic_client_credentials_rebuilds_client(self):
        old_token = _dummy_anthropic_api_key("old")
        new_token = _dummy_anthropic_api_key("new")
        with (
            patch("run_agent.get_tool_definitions", return_value=_make_tool_defs("web_search")),
            patch("run_agent.check_toolset_requirements", return_value={}),
            patch("agent.anthropic_adapter.build_anthropic_client") as mock_build,
        ...
        agent._anthropic_api_key = old_token
        agent._anthropic_base_url = "https://api.anthropic.com"
        agent.provider = "anthropic"

        with (
            patch("agent.anthropic_adapter.resolve_anthropic_token", return_value=new_token),
        ...
        rebuild.assert_called_once_with(
            new_token, "https://api.anthropic.com", timeout=None,
        )
        assert agent._anthropic_client is new_client
        assert agent._anthropic_api_key == new_token
...
class TestOAuthFlagAfterCredentialRefresh:
    ...
    def test_oauth_flag_updates_api_key_to_oauth(self, agent):
        ...
        old_token = _dummy_anthropic_api_key("old")
        new_token = _dummy_anthropic_oauth_token("new")
        agent.api_mode = "anthropic_messages"
        agent.provider = "anthropic"
        agent._anthropic_api_key = old_token
        agent._anthropic_client = MagicMock()
    ...
    def test_oauth_flag_updates_oauth_to_api_key(self, agent):
        ...
        old_token = _dummy_anthropic_oauth_token("old")
        new_token = _dummy_anthropic_api_key("new")
        agent.api_mode = "anthropic_messages"
        agent.provider = "anthropic"
        agent._anthropic_api_key = old_token
...
class TestFallbackSetsOAuthFlag:
    ...
    def test_fallback_to_anthropic_oauth_sets_flag(self, agent):
        ...
        mock_client = MagicMock()
        mock_client.base_url = "https://api.anthropic.com/v1"
        mock_client.api_key = _dummy_anthropic_oauth_token("fallback")
    ...
    def test_fallback_to_anthropic_api_key_clears_flag(self, agent):
        ...
        mock_client = MagicMock()
        mock_client.base_url = "https://api.anthropic.com/v1"
        mock_client.api_key = _dummy_anthropic_api_key("fallback")
