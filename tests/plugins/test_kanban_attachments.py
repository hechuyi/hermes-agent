...
def test_add_attachment_rejects_path_outside_attachments_root(kanban_home, tmp_path):
    conn = kb.connect()
    try:
        task_id = _make_task(conn)
        outside = tmp_path / "outside.txt"
        outside.write_text("must not be surfaced to workers")
...
def test_delete_attachment_does_not_unlink_tampered_outside_path(kanban_home, tmp_path):
    conn = kb.connect()
    try:
        task_id = _make_task(conn)
        outside = tmp_path / "outside.txt"
        ...
        cur = conn.execute(
            ...
            (task_id, "outside.txt", str(outside), "text/plain", outside.stat().st_size, "tester"),
        )
        ...
        assert outside.exists(), "tampered rows must not delete files outside attachments_root"
        assert kb.get_attachment(conn, att_id) is None
    finally:
        conn.close()


def test_delete_task_removes_attachment_rows_and_blobs(kanban_home):
    conn = kb.connect()
    try:
        task_id = _make_task(conn)
        dest_dir = kb.task_attachments_dir(task_id)
        dest_dir.mkdir(parents=True, exist_ok=True)
        blob = dest_dir / "source.txt"
        blob.write_text("data")
        att_id = kb.add_attachment(conn, task_id, filename="source.txt", stored_path=str(blob))
...
def test_delete_archived_task_removes_attachment_rows_and_blobs(kanban_home):
    conn = kb.connect()
    try:
        task_id = _make_task(conn)
        dest_dir = kb.task_attachments_dir(task_id)
...
def test_add_attachment_appends_event(kanban_home):
    ...
    try:
        ...
        blob.write_text("abc")
        kb.add_attachment(
            conn, task_id, filename="a.txt", stored_path=str(blob), size=3
        )
...
def test_attachments_root_is_per_board(kanban_home, monkeypatch):
    ...
    named = kb.attachments_root(board="project-a")
    assert named == kb.board_dir("project-a") / "attachments"
...
def test_upload_does_not_follow_broken_symlink_outside_task_dir(client, tmp_path):
    task_id = _create_task_via_api(client)
    dest_dir = kb.task_attachments_dir(task_id)
    dest_dir.mkdir(parents=True, exist_ok=True)
    ...
    stored_path = Path(r.json()["attachment"]["stored_path"])
    assert stored_path.name == "evil (1).txt"
    assert stored_path.read_bytes() == b"x"
...
def test_download_rejects_tampered_path_outside_task_dir(client):
    first = _create_task_via_api(client)
    second = _create_task_via_api(client)
