...
class TestBaseUrlValidation:
    ...
    MiniMax still uses the generic text input path. Z.AI uses an endpoint
    picker covered by TestZaiEndpointPicker below.
    """
...
class TestZaiEndpointPicker:
    """Z.AI setup should present an endpoint picker."""
    ...
    def test_select_global_endpoint(self, config_home, monkeypatch):
        ...
        assert model["base_url"] == global_url.rstrip("/")
    ...
    def test_select_coding_plan_global_endpoint(self, config_home, monkeypatch):
        ...
        with patch("hermes_cli.main._prompt_provider_choice", return_value=2), \
             patch("hermes_cli.auth._prompt_model_selection", return_value="glm-5.1"), \
             patch("hermes_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
            _model_flow_api_key_provider(load_config(), "zai", "old-model")

        model = load_config()["model"]
        assert model["base_url"] == coding_url.rstrip("/")
    ...
    def test_select_china_endpoint(self, config_home, monkeypatch):
        ...
        model = load_config()["model"]
        assert model["base_url"] == china_url.rstrip("/")
    ...
    def test_current_endpoint_is_default_choice(self, config_home):
        from hermes_cli.auth import ZAI_ENDPOINTS
        from hermes_cli.main import _select_zai_endpoint

        coding_url = ZAI_ENDPOINTS[2][1]
        captured = {}

        def fake_choice(choices, default=0):
        ...
        assert result == coding_url.rstrip("/")

    def test_custom_url_active_defaults_to_custom_option(self, config_home):
        from hermes_cli.auth import ZAI_ENDPOINTS
        from hermes_cli.main import _select_zai_endpoint
