...
def _fake_psutil_with_parent_chain(
    ...
    *,
    ...
    """Build a psutil stand-in that has Process()/parents()/exe().

    ``parent_chain`` is the ordered ancestor list returned by
    ``proc.parents()`` on the seed. ``ancestor_exe`` is the executable path
    reported by each ancestor; ``None`` models an unreadable ancestor exe.
    """
    ...
    class _FakeProc:
        ...
        def parents(self):
            return [_FakeProc(pid, ancestor_exe) for pid in parent_chain]
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    """An unreadable ancestor exe must not crash concurrent detection."""
    scripts_dir = tmp_path
    shim = scripts_dir / "hermes.exe"
    shim.write_bytes(b"")
    me = os.getpid()
