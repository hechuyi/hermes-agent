...
catalog exposes, plus how each provider's ``auth_type`` maps to a provider
bucket — never a specific provider count or a frozen vendor list (both change
over time).
"""
...
def test_copilot_surfaces_as_a_provider_with_its_own_token_var():
    ...
    keys bucket — but its PRIMARY credential var must be the provider-owned
    COPILOT_GITHUB_TOKEN, not the shared tool-category GITHUB_TOKEN. That is what
    lets provider consumers surface Copilot as its own provider.
    """
    by = provider_catalog_by_slug()
...
def test_bedrock_routes_to_keys():
    """Bedrock is aws_sdk (AWS_REGION/AWS_PROFILE), configured in the keys bucket."""
    by = provider_catalog_by_slug()
    assert by["bedrock"].tab == "keys"


def test_api_key_providers_expose_a_credential_env_var():
    """Every keys-bucket provider that authenticates via a pasted API key must
    surface at least one env var to write the key into (otherwise config
    consumers cannot configure it).
