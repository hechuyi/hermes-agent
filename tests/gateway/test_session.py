...
class TestSessionSourceRoundtrip:
    def test_full_roundtrip(self):
        source = SessionSource(
            ...
            chat_id="oc_live_chat_12345",
            chat_name="Feishu Project Group",
            chat_type="group",
            user_id="99",
            user_name="alice",
            thread_id="t1",
        ...
        assert restored.chat_id == "oc_live_chat_12345"
        assert restored.chat_name == "Feishu Project Group"
    ...
    def test_legacy_telegram_numeric_chat_id_coerced_to_string(self):
        """Legacy compatibility: old Telegram session sources may store numeric chat_id."""
...
class TestSessionSourceDescription:
    ...
    def test_dm_with_username(self):
        source = SessionSource(
            platform=Platform.FEISHU, chat_id="ou_private_123",
    ...
    def test_dm_without_username_falls_back_to_user_id(self):
        source = SessionSource(
            platform=Platform.API_SERVER, chat_id="request-123",
    ...
    def test_group_shows_chat_name(self):
        source = SessionSource(
            platform=Platform.FEISHU, chat_id="oc_dev_chat",
    ...
    def test_channel_type(self):
        source = SessionSource(
            platform=Platform.API_SERVER, chat_id="status-feed",
    ...
    def test_thread_id_appended(self):
        source = SessionSource(
            platform=Platform.FEISHU, chat_id="oc_general",
    ...
    def test_unknown_chat_type_uses_name(self):
        source = SessionSource(
            platform=Platform.API_SERVER, chat_id="forum-1",
...
class TestBuildSessionContextPrompt:
    ...
    def test_feishu_prompt_with_channel_topic(self):
    ...
    def test_non_thread_group_shows_user(self):
        ...
        config = GatewayConfig(
            platforms={
                Platform.API_SERVER: PlatformConfig(enabled=True, api_key="fake"),
            },
        )
        source = SessionSource(
...
class TestSessionKeyConsistency:
    ...
    def test_dm_without_chat_id_falls_back_to_user_id(self):
        """A non-Feishu DM source missing chat_id must still isolate by sender."""
        source = SessionSource(
            platform=Platform.API_SERVER,
            chat_id="",
            chat_type="dm",
        ...
        assert build_session_key(source) == "agent:main:api_server:dm:jordan"
    ...
    def test_dm_without_chat_id_distinct_users_do_not_collide(self):
        ...
        assert build_session_key(first) == "agent:main:api_server:dm:jordan"
        assert build_session_key(second) == "agent:main:api_server:dm:dima"
    ...
    def test_dm_without_chat_id_prefers_user_id_alt(self):
        ...
        assert build_session_key(source) == "agent:main:api_server:dm:alt"
    ...
    def test_dm_without_chat_id_or_user_id_falls_back_to_thread_then_sink(self):
        ...
        assert build_session_key(threaded) == "agent:main:api_server:dm:7"
        assert build_session_key(bare) == "agent:main:api_server:dm"

    def test_store_creates_distinct_group_sessions_per_user(self, store):
        store.config.session_isolation_overrides = {"group_sessions_per_user": True}
        store.config._session_isolation_explicit = frozenset({"group_sessions_per_user"})
        first = SessionSource(
            platform=Platform.FEISHU,
        ...
        assert first_entry.session_key == "agent:main:feishu:group:oc_group_123:alice"
        assert second_entry.session_key == "agent:main:feishu:group:oc_group_123:bob"
    ...
    def test_store_shares_group_sessions_when_disabled_in_config(self, store):
        ...
        assert first_entry.session_key == "agent:main:feishu:group:oc_group_123"
        assert second_entry.session_key == "agent:main:feishu:group:oc_group_123"
        assert first_entry.session_id == second_entry.session_id

    def test_feishu_dm_includes_chat_id(self):
        """Runtime DMs include chat_id to separate live conversations."""
        source = SessionSource(
            platform=Platform.FEISHU,
            chat_id="ou_dm_99",
            chat_type="dm",
        ...
        assert key == "agent:main:feishu:dm:ou_dm_99"

    def test_distinct_dm_chat_ids_get_distinct_session_keys(self):
        """Different DM chats must not collapse into one shared session."""
        first = SessionSource(platform=Platform.FEISHU, chat_id="ou_dm_99", chat_type="dm")
        second = SessionSource(platform=Platform.FEISHU, chat_id="ou_dm_100", chat_type="dm")

        assert build_session_key(first) == "agent:main:feishu:dm:ou_dm_99"
        assert build_session_key(second) == "agent:main:feishu:dm:ou_dm_100"
        assert build_session_key(first) != build_session_key(second)
    ...
    def test_feishu_group_includes_chat_id(self):
        ...
        key = build_session_key(source)
        assert key == "agent:main:feishu:group:oc_group_123"
    ...
    def test_group_sessions_are_isolated_per_user_when_user_id_present(self):
        ...
        assert build_session_key(first) == "agent:main:api_server:group:channel-123:alice"
        assert build_session_key(second) == "agent:main:api_server:group:channel-123:bob"
        assert build_session_key(first) != build_session_key(second)
    ...
    def test_group_sessions_can_be_shared_when_isolation_disabled(self):
        ...
        assert build_session_key(first, group_sessions_per_user=False) == "agent:main:api_server:group:channel-123"
        assert build_session_key(second, group_sessions_per_user=False) == "agent:main:api_server:group:channel-123"
    ...
    def test_group_thread_includes_thread_id(self):
        ...
        key = build_session_key(source)
        assert key == "agent:main:feishu:group:oc_group:17585"
    ...
    def test_group_thread_sessions_are_shared_by_default(self):
        ...
        assert build_session_key(alice) == "agent:main:feishu:group:oc_group:17585"
        assert build_session_key(bob) == "agent:main:feishu:group:oc_group:17585"
    ...
    def test_group_thread_sessions_can_be_isolated_per_user(self):
        ...
        assert key == "agent:main:feishu:group:oc_group:17585:42"
    ...
    def test_non_thread_group_sessions_still_isolated_per_user(self):
        ...
        assert build_session_key(alice) == "agent:main:api_server:group:channel-123:alice"
        assert build_session_key(bob) == "agent:main:api_server:group:channel-123:bob"
        assert build_session_key(alice) != build_session_key(bob)

    def test_feishu_thread_sessions_shared_by_default(self):
        """Feishu threads are shared across participants by default."""
        alice = SessionSource(
            platform=Platform.FEISHU,
            chat_id="oc_group_123",
            chat_type="thread",
    ...
    def test_dm_thread_sessions_not_affected(self):
        ...
        assert key == "agent:main:feishu:dm:ou_dm_99:topic-1"
...
class TestHasAnySessions:
    ...
    def test_uses_database_count_when_available(self, store_with_mock_db):
        ...
        store._entries = {"feishu:oc_chat_12345": MagicMock()}
