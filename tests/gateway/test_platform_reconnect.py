...
LIVE_PLATFORM = Platform.FEISHU
HEADLESS_PLATFORM = Platform.API_SERVER
...
def _make_runner():
    ...
    runner.config = GatewayConfig(
        platforms={LIVE_PLATFORM: PlatformConfig(enabled=True, token="test")}
    )
...
class TestStartupPlatformIsolation:
    ...
    async def test_start_continues_after_platform_connect_timeout(self, tmp_path):
        """A timeout on Feishu should queue it and still connect API Server."""
        runner = _make_runner()
        runner.config = GatewayConfig(
            platforms={
                LIVE_PLATFORM: PlatformConfig(enabled=True, token="test"),
                HEADLESS_PLATFORM: PlatformConfig(enabled=True, token="test"),
            },
        ...
        adapters = {
            LIVE_PLATFORM: StubAdapter(platform=LIVE_PLATFORM),
            HEADLESS_PLATFORM: StubAdapter(platform=HEADLESS_PLATFORM),
        }
        ...
        assert HEADLESS_PLATFORM in runner.adapters
    ...
    async def test_connect_adapter_timeout_raises_retryable_exception(self, monkeypatch):
        ...
        with pytest.raises(TimeoutError, match="feishu connect timed out"):
            await runner._connect_adapter_with_timeout(adapter, LIVE_PLATFORM)
...
class TestStartupFailureQueuing:
    ...
    def test_failed_platform_queued_on_connect_failure(self):
        ...
        assert LIVE_PLATFORM in runner._failed_platforms
        assert runner._failed_platforms[LIVE_PLATFORM]["attempts"] == 1
...
class TestPlatformReconnectWatcher:
    ...
    async def test_reconnect_succeeds_on_retry(self):
        ...
        assert LIVE_PLATFORM not in runner._failed_platforms
        assert LIVE_PLATFORM in runner.adapters
    ...
    async def test_reconnect_never_auto_pauses_retryable_failures(self):
        """Retryable failures must keep retrying without manual resume.

        Transient network/DNS outages may last longer than the old circuit
        breaker threshold. Auto-pausing them leaves the platform dead after
        connectivity returns.
        """
        runner = _make_runner()

        platform_config = PlatformConfig(enabled=True, token="test")
        # Far past the old circuit-breaker threshold: retryable failures still
        # must not be paused automatically.
        runner._failed_platforms[LIVE_PLATFORM] = {
            "config": platform_config,
            "attempts": 25,
            "next_retry": time.monotonic() - 1,
        ...
        # Platform stays in queue and keeps retrying — paused only means
        # explicit operator pause.
        assert LIVE_PLATFORM in runner._failed_platforms
        info = runner._failed_platforms[LIVE_PLATFORM]
    ...
    async def test_reconnect_exception_never_auto_pauses_retryable_failures(self):
        """Raised reconnect errors should also stay queued for retry."""
        runner = _make_runner()
    ...
    async def test_reconnect_skips_paused_platforms(self):
        ...
        assert LIVE_PLATFORM in runner._failed_platforms
        assert runner._failed_platforms[LIVE_PLATFORM]["paused"] is True
...
class TestRuntimeDisconnectQueuing:
    ...
    async def test_retryable_runtime_error_queued_for_reconnect(self):
        ...
        runner.adapters[LIVE_PLATFORM] = adapter

        await runner._handle_adapter_fatal_error(adapter)

        assert LIVE_PLATFORM in runner._failed_platforms
        assert runner._failed_platforms[LIVE_PLATFORM]["attempts"] == 0
    ...
    async def test_retryable_error_no_exit_when_other_adapters_still_connected(self):
        ...
        runner.adapters[LIVE_PLATFORM] = failing_adapter

        # Another adapter is still connected
        healthy_adapter = StubAdapter(platform=HEADLESS_PLATFORM, succeed=True)
        runner.adapters[HEADLESS_PLATFORM] = healthy_adapter
...
class TestPauseResume:
    ...
    def test_pause_marks_platform_paused(self):
        ...
        runner._pause_failed_platform(LIVE_PLATFORM, reason="manual")
        info = runner._failed_platforms[LIVE_PLATFORM]
    ...
    def test_pause_is_idempotent(self):
        ...
        runner._pause_failed_platform(LIVE_PLATFORM, reason="second reason")
        # Reason should not be overwritten on a second pause call.
        assert (
            runner._failed_platforms[LIVE_PLATFORM]["pause_reason"]
            == "first reason"
        )
    ...
    def test_pause_no_op_when_platform_not_queued(self):
        ...
        runner._pause_failed_platform(LIVE_PLATFORM, reason="x")
        assert LIVE_PLATFORM not in runner._failed_platforms
    ...
    def test_resume_clears_paused_and_resets_attempts(self):
        ...
        assert runner._resume_paused_platform(LIVE_PLATFORM) is True
        info = runner._failed_platforms[LIVE_PLATFORM]
    ...
    def test_resume_returns_false_when_not_paused(self):
        ...
        assert runner._resume_paused_platform(LIVE_PLATFORM) is False
...
class TestPlatformSlashCommand:
    ...
    async def test_list_shows_connected_and_paused(self):
        runner = _make_runner()
        runner.adapters[HEADLESS_PLATFORM] = StubAdapter(platform=HEADLESS_PLATFORM)
        runner._failed_platforms[LIVE_PLATFORM] = {
            "config": PlatformConfig(enabled=True, token="t"),
            "attempts": 10,
            "next_retry": float("inf"),
    ...
    async def test_resume_command_resumes_paused_platform(self):
        ...
        assert runner._failed_platforms[LIVE_PLATFORM]["paused"] is False
