...
def _delegate_spinner_label(function_args: dict) -> str:
    ...
    if goal_preview:
        return f"🔀 {goal_preview} · (/agents to monitor)"

    return "🔀 delegating · (/agents to monitor)"
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    if not agent.quiet_mode:
        ...
        for i, (tc, name, args, block_result, blocked_by_guardrail) in enumerate(parsed_calls, 1):
            ...
            if agent.verbose_logging:
                ...
                print(f"  📞 Tool {i}: {name}({list(display_args.keys())}) - {args_preview}")
    ...
    def _run_tool(index, tool_call, function_name, function_args):
        ...
        try:
            ...
            # starts with a clean slate. BaseException subclasses bypass
            # ``except Exception`` above, so cleanup has to live in finally.
            with agent._tool_worker_threads_lock:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if not agent.quiet_mode:
            ...
            if agent.verbose_logging:
                ...
                args_preview = args_str[:agent.log_prefix_chars] + "..." if len(args_str) > agent.log_prefix_chars else args_str
                print(f"  📞 Tool {i}: {function_name}({list(display_args.keys())}) - {args_preview}")
        ...
        if _block_msg is not None:
            ...
            spinner_label = _delegate_spinner_label(function_args)
            spinner = None
            if agent._should_emit_quiet_tool_messages() and agent._should_start_quiet_spinner():
                face = random.choice(KawaiiSpinner.get_waiting_faces())
