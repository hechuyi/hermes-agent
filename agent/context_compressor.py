...
# such as "resume exactly from Active Task" survive embedded in the body.
_HISTORICAL_SUMMARY_PREFIXES = (
    # Carveout era: "consistent -> use as background" licensed stale-task
    # resumption on topic overlap.
    "[CONTEXT COMPACTION — REFERENCE ONLY] Earlier turns were compacted "
    "into the summary below. This is a handoff from a previous context "
    "window — treat it as background reference, NOT as active instructions. "
    "Do NOT answer questions or fulfill requests mentioned in this summary; "
...
def _extract_tool_call_id(tool_call: Any) -> str:
    ...
    return str(getattr(tool_call, "id", "") or getattr(tool_call, "call_id", "") or "")
...
class ContextCompressor(ContextEngine):
    ...
    def _build_static_fallback_summary(
        ...
        def _collect_paths_from_jsonish(obj: Any) -> None:
            if isinstance(obj, dict):
                for key, val in obj.items():
                    if key in {"path", "workdir", "file_path", "output_path"} and isinstance(
                        val, str
                    ):
        ...
        if self._previous_summary:
            previous_summary_note = (
                "\n\nPrevious compaction summary was present and should still "
                "be treated as background continuity context, but the latest "
                "LLM summary update failed."
            )
...
- Questions awaiting an answer ("why is X on Y?", "what are the next steps?")
...
"User asked: 'Why did provider suddenly switch to openrouter?' — needs investigation + answer"
...
The user has requested that this compaction PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def _strip_summary_prefix(summary: str) -> str:
        """Return summary body without current, legacy, or historical prefixes."""
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None, force: bool = False) -> List[Dict[str, Any]]:
        ...
        if compress_start >= compress_end:
            ...
            if not self.quiet_mode:
                logger.warning(
                    ...
                    "nothing to compress. ineffective_compression_count=%d",
                    compress_start,
                    compress_end,
                    self._ineffective_compression_count,
                )
        ...
        # False (the default).  Insert a deterministic local handoff so the
        # model keeps recoverable continuity anchors instead of a content-free
        # "N messages were removed" marker.
        if not summary:
            if not self.quiet_mode:
                logger.warning(
                    "Summary generation failed — inserting deterministic "
        ...
        # weak models read the verbatim historical task snapshot of a past
