...
from model_tools import (
    ...
    handle_function_call,  # noqa: F401
...
from agent.codex_responses_adapter import (
    ...
    _summarize_user_message_for_log,  # noqa: F401
)
...
class AIAgent:
    ...
    def _max_tokens_param(self, value: int) -> dict:
        ...
        that kwarg for those families served via their OpenAI-compatible
        endpoints. Third-party gateways can front the same families, so fall
        back to model-name detection when the URL is not enough.
        """
        if (
    ...
    def _provider_supports_vision_tool_messages(self) -> bool:
        ...
        Some providers support multimodal user messages but reject
        list-type tool message content with 400 errors. This checks the
        provider profile's ``supports_vision_tool_messages`` field.
        """
        try:
