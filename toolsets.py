...
_HERMES_CORE_TOOLS = [
    ...
    # Cronjob management
    "cronjob",
    # Feishu/Lark messaging (gated on gateway running via check_fn)
    "send_message",
...
TOOLSETS = {
    ...
    "messaging": {
        "description": "Feishu/Lark messaging: send messages to Feishu chats",
        "tools": ["send_message"],
        "includes": []
    },
    ...
    "hermes-cron": {
        ...
        # excluded by _get_platform_tools() unless
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    ...
    tools = set(toolset.get("tools", [])) - core
    for included_name in toolset.get("includes", []):
        included = get_toolset(included_name)
        if included:
            tools.update(set(included.get("tools", [])) - core)
        else:
            tools.update(set(resolve_toolset(included_name)) - core)
    return tools
